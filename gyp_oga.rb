...
def parse_file io
  ...
  Oga.parse_html(io).css('table:nth-of-type(1) tr').map { |tr_elem|
    if found_strings.include? tr_elem.at_css('img').attribute('title').to_s.downcase
      ...
      cache_link = cache_elem.attribute('href')
